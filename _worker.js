...
case 'list-kv-namespaces': return json(await cfGet(`/accounts/${payload.accountId || await getAccountId(payload.email, payload.key)}/storage/kv/namespaces`, payload.email, payload.key));
      ...
      case 'get-zone-analytics': {
        ...
        try {
          ...
          const q = `query Z($tag:String!,$st:Time!,$et:Time!){viewer{zones(filter:{zoneTag:$tag}){httpRequests1hGroups(limit:24,filter:{datetime_geq:$st,datetime_leq:$et}){dimensions{datetime}sum{requests bytes}uniq{uniques}}}}}`
...
async function cfZonesOfAccount(env, email, key, accountId, cap = 80){
  ...
  if (cached && Date.now() - cached.t < 600000) return { tags: cached.tags, names: cached.names };
  const tags = [], names = {}; let fault = '';
  try {
    for (let pg = 1; pg <= 6; pg++){
      const r = await cfGet('/zones?per_page=50&page=' + pg, email, key).catch(()=>null);
      if (!r || !r.success || !Array.isArray(r.result)) { fault = 'REST /zones: ' + String((r && r.errors && r.errors[0] && r.errors[0].message) || 'request failed').slice(0, 100); break; }
  ...
  if (fault) { if (cached) return { tags: cached.tags, names: cached.names }; return { tags, names, fault }; }
  try { if (env && env.CF_ACCOUNTS_KV) await env.CF_ACCOUNTS_KV.put(ck, JSON.stringify({ t: Date.now(), ok: true, tags, names })); } catch(e){}
  return { tags, names };
}
    ...
    try {
      ...
      for (const zid of zlist.tags) {
        try {
          ...
          if (fj && fj.errors && fj.errors.length) { if (!_anErr) { try { _anErr = String((fj.errors[0] && fj.errors[0].message) || '').slice(0, 140); } catch(e){} } continue; }
          const fz = fj && fj.data && fj.data.viewer && fj.data.viewer.zones && fj.data.viewer.zones[0];
          if (fz) zones.push({ name: zlist.names[fz.zoneTag] || fz.zoneTag || '', zoneTag: fz.zoneTag || zid, httpRequests1hGroups: fz.httpRequests1hGroups || [], country: fz.country || [], status: fz.status || [], colo: fz.colo || [] });
        } catch(e){}
      }
      if (!zones.length && !_anErr) _anErr = zlist.fault || '未取到 zones(逐 zone 查询为空)';
      ...
      for(const z of zones){
        ...
        for(const h of groups){
          ...
          const t = String((h.dimensions && (h.dimensions.datetimeHour || h.dimensions.datetime)) || '').slice(11, 13);
    ...
    try {
      ...
      for (const zid of zlist.tags) {
        try {
          ...
          if (fj && fj.errors && fj.errors.length) { if (!_anErr) { try { _anErr = String((fj.errors[0] && fj.errors[0].message) || '').slice(0, 140); } catch(e){} } continue; }
          const fz = fj && fj.data && fj.data.viewer && fj.data.viewer.zones && fj.data.viewer.zones[0];
          if (fz) zones.push({ zoneTag: fz.zoneTag || zid, httpRequests1dGroups: fz.httpRequests1dGroups || [] });
        } catch(e){}
      }
      if (!zones.length && !_anErr) _anErr = zlist.fault || '未取到 zones(逐 zone 查询为空)';
      const dayMap = {};
...
function renderStaticJS(env) {
  ...
  if (page === 'app') {
    ...
    async function loadZoneAnalytics(){
      ...
      const pts = groups.map(g => ({ t: String((g.dimensions && (g.dimensions.datetimeHour || g.dimensions.datetime)) || '').slice(11, 16), req: (g.sum && g.sum.requests) || 0, bytes: (g.sum && g.sum.bytes) || 0 }));
